...
if [ -f "$ENV_FILE" ]; then
    ...
    if grep -q "^DOCKER_GID=" "$ENV_FILE"; then
        ...
        if [ "$CURRENT_GID" = "$DOCKER_GID" ]; then
            echo "✅ .env already configured correctly (DOCKER_GID=$DOCKER_GID)"
        else
            echo "⚠️  .env exists but DOCKER_GID is different:"
        ...
        echo "⚙️  Adding DOCKER_GID to existing .env"
...
DOCKER_GID=$DOCKER_GID" "$ENV_FILE"
    ...
    if [ -f "$ENV_TEMPLATE" ]; then
        ...
        echo "✅ Created .env from template with DOCKER_GID=$DOCKER_GID"
    else
        # Create from scratch
        cat > "$ENV_FILE" << EOF
# ===================================
...
# ===================================
EOF
        echo "✅ Created .env with DOCKER_GID=$DOCKER_GID"
    fi
fi
...
echo "✅ Created directories: repos/, ssh-keys/, logs/, config/"
...
if command -v docker &> /dev/null; then
    if docker ps &> /dev/null; then
        ...
        if docker compose version &> /dev/null; then
            ...
            echo "⚠️  Docker Compose not found (will use in-container version)"
        fi
    else
    ...
    echo "ℹ️  Docker CLI not found on host (will use in-container version)"
fi
